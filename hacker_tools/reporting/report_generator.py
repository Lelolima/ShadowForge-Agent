...
from core.state import EstadoAgente, VulnerabilidadeDescoberta  # L-08 FIX: Tipo específico em vez de Any
...
class ResumoExecutivo(TypedDict):
    objetivo: str
    periodo: str
...
class Escopo(TypedDict):
    alvo_principal: str | None
    total_hosts: int
    hosts: List[HostItem]
    tipo_teste: str
    restricoes: str
...
class Metodologia(TypedDict):
    frameworks: List[str]
    fases: List[dict[str, str]]
    ferramentas: List[str]


class VulnerabilidadeDetalhe(TypedDict):
    id: str
    titulo: str
    tipo: str
    severidade: str
...
class Apendices(TypedDict):
    ferramentas: List[str]
    total_acoes: int
    duracao_min: int


class ImpactoCIA(TypedDict):
...
class RiscoResidual(TypedDict):
    pos_remediacao: str
    recomendacoes_adicionais: List[str]


class RelatorioCompleto(TypedDict):
    meta: RelatorioMeta
    resumo_executivo: ResumoExecutivo
    escopo: Escopo
    metodologia: Metodologia
    vulnerabilidades: List[VulnerabilidadeDetalhe]
    score_geral: float
    recomendacoes: List[str]
    risco_residual: RiscoResidual
    apendices: Apendices
...
class ReportGenerator:
    ...
    async def gerar(self, estado: EstadoAgente) -> RelatorioCompleto:
    ...
    def _gerar_resumo_executivo(self, estado: EstadoAgente) -> ResumoExecutivo:
    ...
    def _gerar_escopo(self, estado: EstadoAgente) -> Escopo:
    ...
    def _gerar_metodologia(self) -> Metodologia:
    ...
    def _detalhar_vulnerabilidades(self, estado: EstadoAgente) -> List[VulnerabilidadeDetalhe]:
    ...
    def _avaliar_impacto(self, vuln: VulnerabilidadeDescoberta) -> ImpactoCIA:
    ...
    def _gerar_recomendacao(self, vuln: VulnerabilidadeDescoberta) -> str:
    ...
    def _gerar_referencias(self, vuln: VulnerabilidadeDescoberta) -> List[str]:
    ...
    def _calcular_score_geral(self, vulnerabilidades: List[VulnerabilidadeDescoberta]) -> float:
    ...
    def _avaliar_risco_residual(self, estado: EstadoAgente) -> RiscoResidual:
    ...
    def exportar_markdown(self, relatorio: RelatorioCompleto) -> str:
        ...
        md += "\n---\n*Gerado automaticamente por SH4icamente por SH4D0WF0RG3*\n"
        return md

    def exportar_json(self, relatorio: RelatorioCompleto) -> str:
    ...
    def exportar_html(self, relatorio: RelatorioCompleto) -> str:
...
<p>{self._esc(resumo_exec['conclusao'])}</p>
<p><span class="label">Vulnerabilidades:</span> <span class="value">{self._esc(resumo_exec['total_vulnerabilidades'])}</span> |
<span class="label">Críticas:</span> <span class="value" style="color:var(--red)">{self._esc(resumo_exec['criticas'])}</span> |
<span class="label">Altas:</span> <span class="value" style="color:#ff6600">{self._esc(resumo_exec['altas'])}</span></p>
<p><span class="label">Nível de Risco:</span> <span class="value" style="color:var(--red);font-weight:bold">{self._esc(resumo_exec['nivel_risco'])}</span></p>
