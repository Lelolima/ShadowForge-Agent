...
class StealthShell:
    ...
    async def executar(
        ...
        *args: str,
        ...
        Para maior segurança, use o parâmetro _auto_escape=True para escapar
        automaticamente todos os argumentos. Quando _auto_escape=True, o primeiro
        argumento é tratado como o comando e os subsequentes como seus argumentos,
        com cada um sendo escapado individualmente via _safe_quote().

        Args:
            comando: Comando para executar (ou nome do comando quando _auto_escape=True)
            *args: Argumentos para o comando (apenas quando _auto_escape=True)
            ...
            _auto_escape: Se True, escapa automaticamente todos os argumentos
                         usando _safe_quote() para melhor segurança (padrão: False)
        ...
        # Process arguments and apply auto-escaping if requested
        exec_comando = comando
        if _auto_escape:
            # Combine all arguments and escape each one
            all_args = [comando] + list(args)
            escaped_args = [_safe_quote(arg, self._is_windows) for arg in all_args]
            exec_comando = " ".join(escaped_args)

        # H-11 FIX: Rejeitar comandos perigosos da blacklist
        cmd_lower = exec_comando.lower().strip()
        for dangerous in _DANGEROUS_COMMANDS:
            if dangerous in cmd_lower:
                logger.error("[SHELL] Comando bloqueado (blacklist): %s", exec_comando[:80])
        ...
        if shell is None:
            if self._is_windows:
                executable = "powershell.exe" if "|" in exec_comando or ";" in exec_comando else None
            else:
                executable = "/bin/bash"
        else:
            executable = shell
        ...
        try:
            ...
            try:
                ...
                logger.warning("Comando timeout: %s (%ds)", exec_comando[:50], timeout_s)

        except FileNotFoundError as e:
            resultado.stderr = f"Comando não encontrado: {e}"
            resultado.returncode = 127
        except (OSError, subprocess.SubprocessError) as e:
            resultado.stderr = f"Erro de execução: {e}"
            resultado.returncode = -1
        except Exception as e:
            # Fallback for unexpected errors, but log the type for debugging
            resultado.stderr = f"Erro inesperado ({type(e).__name__}): {e}"
            resultado.returncode = -1
