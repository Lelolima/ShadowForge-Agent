...
class PluginManager:
    ...
    async def load_all(self) -> None:
        ...
        for info in self._ordenar_por_dependencias(infos):
    ...
    @staticmethod
    def _ordenar_por_dependencias(infos: list[PluginInfo]) -> list[PluginInfo]:
        """Ordena plugins por dependências com detecção de ciclo.

        H-08 FIX: Detecta dependências circulares para evitar deadlock/loop infinito.
        M-07 FIX: Registro em sys.modules com namespace prefixado.
        """
        resolved = set()
        pending = list(infos)
        ...
        while pending:
            ...
            for info in list(pending):
                if all(d in resolved for d in info.dependencias):
                    ordered.append(info)
                    resolved.add(info.nome)
                    pending.remove(info)
                    progress = True

            # H-08 FIX: Se nenhuma dependência foi resolvida nesta iteração, há ciclo
            if not progress and pending:
                nomes_ciclo = [info.nome for info in pending]
                deps_ciclo = {info.nome: info.dependencias for info in pending}
                logger.error(
                    "Dependência circular detectada entre plugins: %s (deps: %s). "
                    "Carregando plugins restantes sem ordenação.",
                    nomes_ciclo, deps_ciclo,
                )
                # Adiciona os restantes sem ordenação para evitar loop infinito
